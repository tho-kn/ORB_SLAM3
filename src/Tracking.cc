...
#include <chrono>
#include <include/CameraModels/DoubleSphere.h>
...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
    ...
    : mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false),
      ...
      time_recently_lost_visual(2.0), mnInitialFrameId(0), mbCreatedMap(false),
      mnFirstFrameId(0), mpCamera2(nullptr) {
  // Load camera parameters from settings file
  cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);
...
void Tracking::LocalMapStats2File() {
  ...
  for (int i = 0; i < mpLocalMapper->vdLMTotal_ms.size(); ++i) {
    f << mpLocalMapper->vdKFInsert_ms[i] << ","
      ...
      << "," << mpLocalMapper->vdKFCulling_ms[i] << ","
...
void Tracking::TrackStats2File() {
  ...
  f << "#KF insert[ms], ORB ext[ms], Stereo match[ms], IMU preint[ms], Pose "
       "pred[ms], LM track[ms], KF dec[ms], Total[ms]"
    << endl;
...
>>>>>>> 4452a3c4ab75b1cde34e5505a36ec3f9edcdc4c4
    ...
    f << stereo_rect << "," << vdORBExtract_ms[i] << "," << stereo_match << ","
      << imu_preint << "," << vdPosePred_ms[i] << "," << vdLMTrack_ms[i] << ","
      << vdNewKF_ms[i] << "," << vdTrackTotal_ms[i] << endl;
  }
  ...
  f << "# number of KF, number of MP, UpdateLM[ms], SearchLP[ms], PoseOpt[ms]"
    << endl;

  for (int i = 0; i < vnKeyFramesLM.size(); ++i) {

    f << vnKeyFramesLM[i] << "," << vnMapPointsLM[i] << "," << vdUpdatedLM_ms[i]
      << "," << vdSearchLP_ms[i] << "," << vdPoseOpt_ms[i] << endl;
  }
...
void Tracking::PrintTimeStats() {
  ...
  std::cout << "---------------------------" << std::endl;
  std::cout << std::endl
            << "Local Map Tracking complexity (mean$\\pm$std)" << std::endl;
  f << "---------------------------" << std::endl;
  f << std::endl
    << "Local Map Tracking complexity (mean$\\pm$std)" << std::endl;
  ...
  std::cout << std::endl << "Map complexity" << std::endl;
  std::cout << "KFs in map: "
            << mpAtlas->GetAllMaps()[0]->GetAllKeyFrames().size() << std::endl;
  std::cout << "MPs in map: "
            << mpAtlas->GetAllMaps()[0]->GetAllMapPoints().size() << std::endl;
  f << "---------------------------" << std::endl;
  f << std::endl << "Map complexity" << std::endl;
  f << "KFs in map: " << mpAtlas->GetAllMaps()[0]->GetAllKeyFrames().size()
    << std::endl;
  f << "MPs in map: " << mpAtlas->GetAllMaps()[0]->GetAllMapPoints().size()
    << std::endl;
...
<<<<<<< HEAD
  ...
  } else if (sCameraName == "DoubleSphere") {
    float fx, fy, cx, cy;
    float xi, alpha;
    ...
    // Distortion parameters
    node = fSettings["Camera.xi"];
    if (!node.empty() && node.isReal()) {
      xi = node.real();
    } else {
      std::cerr << "*Camera.xi parameter doesn't exist or is not a real number*"
                << std::endl;
      b_miss_params = true;
    }
    node = fSettings["Camera.alpha"];
    if (!node.empty() && node.isReal()) {
      alpha = node.real();
    } else {
      std::cerr
          << "*Camera.alpha parameter doesn't exist or is not a real number*"
          << std::endl;
      b_miss_params = true;
    }

    if (!b_miss_params) {
      vector<float> vCamCalib{fx, fy, cx, cy, xi, alpha};
      mpCamera = new DoubleSphere(vCamCalib);
    ...
    if (mSensor == System::STEREO || mSensor == System::IMU_STEREO) {
      ...
      // Distortion parameters
      node = fSettings["Camera2.xi"];
      if (!node.empty() && node.isReal()) {
        xi = node.real();
      } else {
        std::cerr
            << "*Camera2.xi parameter doesn't exist or is not a real number*"
            << std::endl;
        b_miss_params = true;
      }
      node = fSettings["Camera2.alpha"];
      if (!node.empty() && node.isReal()) {
        alpha = node.real();
      } else {
        std::cerr
            << "*Camera2.alpha parameter doesn't exist or is not a real number*"
            << std::endl;
        b_miss_params = true;
      }
...
>>>>>>> 4452a3c4ab75b1cde34e5505a36ec3f9edcdc4c4
      ...
      if (!b_miss_params) {
        static_cast<DoubleSphere *>(mpCamera)->mvLappingArea[0] =
            leftLappingBegin;
        static_cast<DoubleSphere *>(mpCamera)->mvLappingArea[1] =
            leftLappingEnd;

        mpFrameDrawer->both = true;

        vector<float> vCamCalib2{fx, fy, cx, cy, xi, alpha};
        mpCamera2 = new DoubleSphere(vCamCalib2);

        static_cast<DoubleSphere *>(mpCamera2)->mvLappingArea[0] =
            rightLappingBegin;
        static_cast<DoubleSphere *>(mpCamera2)->mvLappingArea[1] =
            rightLappingEnd;
...
>>>>>>> 4452a3c4ab75b1cde34e5505a36ec3f9edcdc4c4
            ...
            bOK = Relocalization();
            if (!bOK && mCurrentFrame.mTimeStamp - mTimeStampLost >
                            time_recently_lost_visual) {
              mState = LOST;
              Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
              bOK = false;
            }
...
void Tracking::CreateMapInAtlas() {
  ...
  mVelocity = cv::Mat();
  mnLastRelocFrameId =
      mnLastInitFrameId; // The last relocation KF_id is the current id, because
                         // it is the new starting point for new map
...
#ifdef REGISTER_TIMES
  std::chrono::steady_clock::time_point time_StartLMUpdate =
      std::chrono::steady_clock::now();
#endif
  UpdateLocalMap();
#ifdef REGISTER_TIMES
  std::chrono::steady_clock::time_point time_StartSearchLP =
      std::chrono::steady_clock::now();

  double timeUpdatedLM_ms =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
          time_StartSearchLP - time_StartLMUpdate)
          .count();
...
>>>>>>> 4452a3c4ab75b1cde34e5505a36ec3f9edcdc4c4
  ...
  SearchLocalPoints();
#ifdef REGISTER_TIMES
  std::chrono::steady_clock::time_point time_StartPoseOpt =
      std::chrono::steady_clock::now();

  double timeSearchLP_ms =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
          time_StartPoseOpt - time_StartSearchLP)
          .count();
  ...
  double timePoseOpt_ms =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
          time_EndPoseOpt - time_StartPoseOpt)
          .count();
